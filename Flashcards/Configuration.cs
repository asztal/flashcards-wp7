	...
	public static class Configuration {
		...
		static Configuration() {
			if (System.ComponentModel.DesignerProperties.IsInDesignTool) {
				Default = new NullConfiguration();
				return;
			}

			Default = new JsonConfiguration();
		}
	...
	public class JsonConfiguration
		...
		void Load() {
			using (var ifs = IsolatedStorageFile.GetUserStoreForApplication()) {
				if (!ifs.FileExists(path))
					return;

				JsonDictionary dict;

				try {
					using (var stream = ifs.OpenFile(path, FileMode.Open, FileAccess.Read, FileShare.Read))
		...
		public void Save() {
			...
			using (var ifs = IsolatedStorageFile.GetUserStoreForApplication()) {
				using (var stream = ifs.OpenFile(path, FileMode.Create, FileAccess.Write, FileShare.None)) {
		...
		public T Get<T>(string setting, T defaultValue) {
			lock (values) {
				// For now, values that are group, but are group to null, are treated as unset.
